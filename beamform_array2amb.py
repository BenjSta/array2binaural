...
def overlap_add(frames, hop_length):
    ...
    y = np.zeros(
        frames_channels_shape
        + tuple(
            [
                (frames.shape[-1] - 1) * hop_length + frames.shape[-2],
            ]
        )
    )
...
NFFT = 1024
d = np.fft.rfft(array_sh, NFFT, axis=0)
# first order encoder
fvec = np.arange(d.shape[0]) / NFFT * fs
d = d * np.exp(1j * 2 * np.pi * fvec * array_sh_delay / fs)[:, None, None]

# foa encoder
dfoa = d[:, :4, :].transpose(0, 2, 1)
dcov = np.conj(dfoa[:, :, [0, 1, 3]].transpose(0, 2, 1)) @ dfoa[:, :, [0, 1, 3]]
...
ewxy[:, [0, 1, 3], :] = e
e = ewxy
e = e * 2 / np.linalg.norm(e @ d.transpose(0, 2, 1), "fro", axis=(1, 2))[:, None, None]
foa_enc = e

foa_enc_td = np.fft.irfft(foa_enc, axis=0)
foa_enc_td = np.roll(foa_enc_td, FILTLEN // 2, axis=0)
foa_enc_td = (
    foa_enc_td[:FILTLEN, :, :] * signal.windows.tukey(FILTLEN, 0.1)[:, None, None]
)
...
for recording_rot in ["static", "dynamic"]:
    ...
    xyz_bfbr = np.stack(
        spaudiopy.utils.sph2cart(directions_bfbr[0, :], directions_bfbr[1, :]), -1
    )
    ...
    for i in range(Y_bfbr.shape[1]):
        D_bfbr.append(np.sum(Y_bfbr[:, None, i, :, None] * d[None, :, :, :], -2))
    ...
    W_bfbr = np.conj(
        ...
        ..., 0
    ]  # ) / np.linalg.norm(D_bfbr, 2, -1, keepdims=True)**2#(1/np.sqrt(D_bfbr.shape[-2]) * u @ vh).transpose(0, 1, 3, 2)
    ...
    for scenario in ["string_quartet", "two_speakers_close", "two_speakers_opposite"]:
        if scenario == "two_speakers_close":
            directions = np.pi / 180 * np.array([[-30, 90], [0, 90]]).T
            xyz = np.stack(
                spaudiopy.utils.sph2cart(directions[0, :], directions[1, :]), -1
            )

        elif scenario == "string_quartet":
            directions = (
                np.pi / 180 * np.array([[-90, 90], [-30, 90], [30, 90], [90, 90]]).T
            )
            xyz = np.stack(
                spaudiopy.utils.sph2cart(directions[0, :], directions[1, :]), -1
            )
        ...
        for i in range(Y.shape[1]):
            D_full.append(np.sum(Y[:, None, i, :, None] * d[None, :, :, :], -2))
        ...
        for reverb in ["anech", "strongrev"]:
            ...
            mic_frames = librosa.util.frame(
                np.pad(mic, ((HOPSIZE, HOPSIZE), (0, 0))).T,
                frame_length=WINLEN,
                hop_length=HOPSIZE,
            ).T
            ...
            for discrete_beams in ["none", "incomplete", "complete"]:
                ...
                if discrete_beams != "none":
                    ...
                    y = np.sum(
                        signal.fftconvolve(
                            sepfilt_td, windowed_mic_frames[:, :, None, :], axes=1
                        ),
                        axis=-1,
                    )
                    hoa = np.sum(Y_norot[None, None, :, :] * y[..., None], -2)
                    hoa_td = overlap_add(hoa.T, HOPSIZE)[
                        ..., HOPSIZE : HOPSIZE + mic.shape[0]
                    ].T
                    amb_mic = np.sum(
                        signal.fftconvolve(
                            resfilt_td, windowed_mic_frames[:, :, None, :], axes=1
                        ),
                        axis=-1,
                    )
                    soundfile.write(
                ...
                amb_mic_td = overlap_add(amb_mic.T, HOPSIZE)[
                    :, HOPSIZE : HOPSIZE + mic.shape[0]
                ].T
                ...
                for ambient_rendering in ["foa", "bfbr", "emagls2"]:
                    if ambient_rendering == "foa":
                        foa = np.sum(
                            signal.fftconvolve(
                                foa_enc_td[None, :, :, :], amb_mic[..., None, :], axes=1
                            ),
                            axis=-1,
                        )
                        foa = (inv_rotmat1[:, None, :, :] @ foa[..., None])[..., 0]
                        foa_td = overlap_add(foa.T, HOPSIZE)[
                            :, HOPSIZE : HOPSIZE + mic.shape[0]
                        ].T
                        soundfile.write(
                            os.path.join(
                        ...
                        enc_bfbr_td = overlap_add(enc_bfbr.T, HOPSIZE)[
                            :, HOPSIZE : HOPSIZE + mic.shape[0]
                        ].T
                        soundfile.write(
                            os.path.join(
            ...
            if recording_rot == "static":
                ref, fs_ref = soundfile.read(
                    os.path.join(
                        "simulate_scenarios_and_mic_signals/audio_o25",
