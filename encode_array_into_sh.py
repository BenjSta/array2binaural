...
Y_gr_pinv = (
    np.linalg.inv(
        Y_gr.T @ weights @ Y_gr +
        np.max(e) * REGUL_DIAG_LOAD * np.eye(Y_gr.shape[1])
    )
    @ Y_gr.T @ weights
)
...
if False:
    ...
    b, a = scipy.signal.butter(
        2, (FMIN_BANDPASS, FMAX_BANDPASS), "bandpass", fs=FS)
    VMIN = -60
    VMAX = -30
    ...
    plt.scatter(
        ...
        c=10
        * np.log10(
            np.mean(scipy.signal.lfilter(
                b, a, ir[:, :, MIC], axis=0) ** 2, axis=0)
        ),
        vmin=VMIN,
        vmax=VMAX,
    )
...
np.save("Easycom_array_%dHz_o%d_22samps_delay.npy" %
        (FS, INTERP_ORDER), hoa_array)
